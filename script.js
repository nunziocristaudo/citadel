...
async function fetchImages() {
  try {
    ...
    if (images.length === 0) {
      console.error("No valid images/videos found in /images folder.");
    } else {
      console.log(`Loaded ${images.length} images/videos.`);
    }
...
// Dynamic grid loading on scroll
...
function loadInitialTiles() {
  ...
  const buffer = window.innerWidth < 768 ? 3 : 5; // Smaller on mobile
